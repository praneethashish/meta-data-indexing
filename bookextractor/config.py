...
class Settings:
    ...
    # ── Paths ──────────────────────────────────────────────
    UPLOAD_DIR: str = os.getenv("BOOKEXTRACTOR_UPLOAD_DIR", "uploads")
    MODELS_DIR: str = os.getenv("BOOKEXTRACTOR_MODELS_DIR", str(Path(__file__).resolve().parents[1] / "models"))
    ...
    # ── vLLM Model ─────────────────────────────────────────
    VLLM_MODEL_ID: str | None = os.getenv("VLLM_MODEL_ID")
    ...
    VISION_MAX_TOKENS: int = int(os.getenv("VISION_MAX_TOKENS", "512"))

    # ── Text Snippet Lengths ───────────────────────────────
    TEXT_SNIPPET_LENGTH: int = 500
    BOOK_EXTRACTION_MAX_LENGTH: int = 15000
    MAGAZINE_EXTRACTION_MAX_LENGTH: int = 3000
    DESCRIPTION_FALLBACK_LENGTH: int = 200

    # ── Celery ─────────────────────────────────────────────
    CELERY_BROKER_URL: str = os.getenv("CELERY_BROKER_URL", "redis://localhost:6379/0")
    CELERY_RESULT_BACKEND: str = os.getenv("CELERY_RESULT_BACKEND", "redis://localhost:6379/0")
    CELERY_TASK_TIME_LIMIT: int = int(os.getenv("CELERY_TASK_TIME_LIMIT", "3600"))
    CELERY_TASK_SOFT_TIME_LIMIT: int = int(os.getenv("CELERY_TASK_SOFT_TIME_LIMIT", "3000"))
    ...
    CELERY_RESULT_EXPIRES: int = int(os.getenv("CELERY_RESULT_EXPIRES", "86400"))
    CELERY_DEFAULT_QUEUE: str = os.getenv("CELERY_DEFAULT_QUEUE", "default_queue")
    CELERY_TASK_ALWAYS_EAGER: bool = os.getenv("CELERY_TASK_ALWAYS_EAGER", "false").lower() == "true"

    # ── Worker ─────────────────────────────────────────────
    STALE_FILE_THRESHOLD: int = int(os.getenv("STALE_FILE_THRESHOLD", "86400"))
    DEFAULT_WORKER_CONCURRENCY: int = int(os.getenv("DEFAULT_WORKER_CONCURRENCY", "4"))

    # ── External Services ──────────────────────────────────
    VPARSE_API_URL: str = os.getenv("VPARSE_API_URL", "http://localhost:8000/file_parse")
    VPARSE_TIMEOUT: float = float(os.getenv("VPARSE_TIMEOUT", "900"))
